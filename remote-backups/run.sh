...
SSH_OPTIONS=$(jq --raw-output ".ssh_options" $CONFIG_PATH)
...
function copy-backup-to-remote {
    ...
    if [[ -z $ZIP_PASSWORD  ]]; then
      echo "Copying ${slug}.tar to ${REMOTE_DIRECTORY} on ${SSH_HOST} using SCP ${SSH_OPTIONS} - DesaiSamir"
      scp "${SSH_OPTIONS}" "${HOME}/.ssh/config" "${slug}.tar" remote:"${REMOTE_DIRECTORY}"
    else
      echo "Copying password-protected ${slug}.zip to ${REMOTE_DIRECTORY} on ${SSH_HOST} using SCP ${SSH_OPTIONS}"
      zip -P "$ZIP_PASSWORD" "${slug}.zip" "${slug}".tar
      scp "${SSH_OPTIONS}" "${HOME}/.ssh/config" "${slug}.zip" remote:"${REMOTE_DIRECTORY}" && rm "${slug}.zip"
    fi
